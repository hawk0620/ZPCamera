...
NSString * const RecordSessionSegmentFilenamesKey = @"RecordSegmentFilenames";
NSString * const RecordSessionSegmentsKey = @"Segments";
NSString * const RecordSessionSegmentFilenameKey = @"Filename";
NSString * const RecordSessionSegmentInfoKey = @"Info";
...
- (id)initWithDictionaryRepresentation:(NSDictionary *)dictionaryRepresentation {
    ...
    if (self) {
        ...
        NSArray *recordSegments = [dictionaryRepresentation objectForKey:RecordSessionSegmentFilenamesKey];
        ...
        for (NSObject *recordSegment in recordSegments) {
            ...
            if ([recordSegment isKindOfClass:[NSDictionary class]]) {
                filename = ((NSDictionary *)recordSegment)[RecordSessionSegmentFilenameKey];
                info = ((NSDictionary *)recordSegment)[RecordSessionSegmentInfoKey];
            } else if ([recordSegment isKindOfClass:[NSString class]]) {
                // EVEN OLDER WAY
                filename = (NSString *)recordSegment;
            }
            
            NSURL *url = [RecordSessionSegment segmentURLForFilename:filename andDirectory:_segmentsDirectory];
            
            if ([[NSFileManager defaultManager] fileExistsAtPath:url.path]) {
                [_segments addObject:[RecordSessionSegment segmentWithURL:url info:info]];
            } else {
        ...
        // NEW WAY
        NSArray *segments = [dictionaryRepresentation objectForKey:RecordSessionSegmentsKey];
        for (NSDictionary *segmentDictRepresentation in segments) {
            RecordSessionSegment *segment = [[RecordSessionSegment alloc] initWithDictionaryRepresentation:segmentDictRepresentation directory:_segmentsDirectory];
        ...
        if (shouldRecomputeDuration) {
            ...
            if (CMTIME_IS_INVALID(_segmentsDuration)) {
                ...
                NSLog(@"The imported RecordSession is probably not useable.");
            }
        }
...
- (id)init {
    ...
    if (self) {
        ...
        _identifier = [NSString stringWithFormat:@"%@-", [RecordSession newIdentifier:12]];
...
+ (id)recordSession {
    return [[RecordSession alloc] init];
}

+ (id)recordSession:(NSDictionary *)dictionaryRepresentation {
    return [[RecordSession alloc] initWithDictionaryRepresentation:dictionaryRepresentation];
}

+ (NSError*)createError:(NSString*)errorDescription {
    return [NSError errorWithDomain:@"RecordSession" code:200 userInfo:@{NSLocalizedDescriptionKey : errorDescription}];
}

- (void)dispatchSyncOnSessionQueue:(void(^)())block {
    RecoderManager *recorder = self.recorder;
    
    if (recorder == nil || [RecoderManager isSessionQueue]) {
        block();
    } else {
...
- (void)removeSegment:(RecordSessionSegment *)segment {
...
- (void)removeSegmentAtIndex:(NSInteger)segmentIndex deleteFile:(BOOL)deleteFile {
    [self dispatchSyncOnSessionQueue:^{
        RecordSessionSegment *segment = [_segments objectAtIndex:segmentIndex];
...
//            NSLog(@"Removed duration of %fs", CMTimeGetSeconds(segmentDuration));
            ...
            for (RecordSessionSegment *segment in _segments) {
...
- (void)removeAllSegments:(BOOL)removeFiles {
    [self dispatchSyncOnSessionQueue:^{
        while (_segments.count > 0) {
            if (removeFiles) {
                RecordSessionSegment *segment = [_segments objectAtIndex:0];
                [segment deleteFile];
            }
...
- (NSURL *)nextFileURL:(NSError **)error {
    ...
    if (extension != nil) {
        ...
        NSURL *file = [RecordSessionSegment segmentURLForFilename:filename andDirectory:self.segmentsDirectory];
        ...
        if (error != nil) {
            *error = [RecordSession createError:[NSString stringWithFormat:@"Unable to find an extension"]];
        }
...
- (AVAssetWriter *)createWriter:(NSError **)error {
    ...
    if (fileType != nil) {
        ...
        if (file != nil) {
            ...
            writer.metadata = [RecorderTools assetWriterMetadata];
        }
    } else {
        theError = [RecordSession createError:@"No fileType has been set in the RecordSession"];
    }
    ...
    if (theError == nil) {
        ...
        if (_videoInput != nil) {
            if ([writer canAddInput:_videoInput]) {
                ...
                theError = [RecordSession createError:@"Cannot add videoInput to the assetWriter with the currently applied settings"];
            }
        }
        ...
        if (_audioInput != nil) {
            if ([writer canAddInput:_audioInput]) {
                ...
                theError = [RecordSession createError:@"Cannot add audioInput to the assetWriter with the currently applied settings"];
            }
        }
...
- (void)initializeVideo:(NSDictionary *)videoSettings formatDescription:(CMFormatDescriptionRef)formatDescription error:(NSError *__autoreleasing *)error {
    ...
    @try {
        ...
        _videoInput.transform = CGAffineTransformRotate(_videoConfiguration.affineTransform, -M_PI_2);//_videoConfiguration.affineTransform;
        ...
        theError = [RecordSession createError:exception.reason];
    }
...
- (void)addSegment:(RecordSessionSegment *)segment {
    [self dispatchSyncOnSessionQueue:^{
        [_segments addObject:segment];
...
- (void)insertSegment:(RecordSessionSegment *)segment atIndex:(NSInteger)segmentIndex {
    [self dispatchSyncOnSessionQueue:^{
...
- (void)beginSegment:(NSError**)error {
    [self dispatchSyncOnSessionQueue:^{
        if (_assetWriter == nil) {
            ...
            if (error != nil) {
                *error = [RecordSession createError:@"A record segment has already began."];
            }
        }
    }];
}
...
- (void)appendRecordSegmentUrl:(NSURL *)url info:(NSDictionary *)info error:(NSError *)error completionHandler:(void (^)(RecordSessionSegment *, NSError *))completionHandler {
    [self dispatchSyncOnSessionQueue:^{
        RecordSessionSegment *segment = nil;
        
        if (error == nil) {
            segment = [RecordSessionSegment segmentWithURL:url info:info];
...
- (BOOL)endSegmentWithInfo:(NSDictionary *)info completionHandler:(void(^)(RecordSessionSegment *segment, NSError* error))completionHandler {
    ...
    [self dispatchSyncOnSessionQueue:^{
        dispatch_sync(_audioQueue, ^{
            if (_recordSegmentReady) {
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    if (completionHandler != nil) {
                        completionHandler(nil, [RecordSession createError:@"The current record segment is not ready for this operation"]);
                    }
                });
            }
        });
...
- (void)beginRecordSegmentUsingMovieFileOutput:(AVCaptureMovieFileOutput *)movieFileOutput error:(NSError *__autoreleasing *)error delegate:(id<AVCaptureFileOutputRecordingDelegate>)delegate {
    ...
    if (url != nil) {
        ...
        _movieFileOutput.metadata = [RecorderTools assetWriterMetadata];
...
- (AVAssetExportSession *)mergeSegmentsUsingPreset:(NSString *)exportSessionPreset completionHandler:(void(^)(NSURL *outputUrl, NSError *error))completionHandler {
    ...
    [self dispatchSyncOnSessionQueue:^{
        ...
        if (fileType == nil) {
            error = [RecordSession createError:@"No output fileType was set"];
            return;
        }
        
        NSString *fileExtension = [self _suggestedFileExtension];
        if (fileExtension == nil) {
            error = [RecordSession createError:@"Unable to figure out a file extension"];
            return;
        }
        
        NSString *filename = [NSString stringWithFormat:@"%@SCVideo-Merged.%@", _identifier, fileExtension];
        outputUrl = [RecordSessionSegment segmentURLForFilename:filename andDirectory:_segmentsDirectory];
        [self removeFile:outputUrl];

        if (_segments.count == 0) {
            error = [RecordSession createError:@"The session does not contains any record segment"];
        } else {
...
- (void)cancelSession:(void (^)())completionHandler {
    [self dispatchSyncOnSessionQueue:^{
        if (_assetWriter == nil) {
            ...
            [self endSegmentWithInfo:nil completionHandler:^(RecordSessionSegment *segment, NSError *error) {
                [self removeAllSegments];
                if (completionHandler != nil) {
                    completionHandler();
                }
...
- (void)appendSegmentsToComposition:(AVMutableComposition *)composition audioMix:(AVMutableAudioMix *)audioMix {
    [self dispatchSyncOnSessionQueue:^{
        ...
        for (RecordSessionSegment *recordSegment in _segments) {
...
- (AVPlayerItem *)playerItemRepresentingSegments {
    ...
    [self dispatchSyncOnSessionQueue:^{
        if (_segments.count == 1) {
            RecordSessionSegment *segment = _segments.firstObject;
...
- (NSDictionary *)dictionaryRepresentation {
    ...
    for (RecordSessionSegment *recordSegment in self.segments) {
        [recordSegments addObject:recordSegment.dictionaryRepresentation];
    }
    
    return @{
             RecordSessionSegmentsKey: recordSegments,
...
- (NSURL *)outputUrl {
    ...
    return [RecordSessionSegment segmentURLForFilename:filename andDirectory:_segmentsDirectory];
}
...
- (void)setSegmentsDirectory:(NSString *)segmentsDirectory {
    ...
    [self dispatchSyncOnSessionQueue:^{
        ...
        for (RecordSessionSegment *recordSegment in self.segments) {
            NSURL *newUrl = [RecordSessionSegment segmentURLForFilename:recordSegment.url.lastPathComponent andDirectory:_segmentsDirectory];
