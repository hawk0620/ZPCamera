...
extern NSString *__nonnull const RecordSessionSegmentFilenameKey;
extern NSString *__nonnull const RecordSessionSegmentFilenamesKey;
extern NSString *__nonnull const RecordSessionSegmentsKey;
extern NSString *__nonnull const SCRecordSessionDurationKey;
extern NSString *__nonnull const SCRecordSessionIdentifierKey;
extern NSString *__nonnull const RecordSessionSegmentInfoKey;
...
/**
 ...
 If null, the RecordSession will figure out one depending on the fileType.
 */
...
/**
 Contains every record segment as RecordSessionSegment.
 */
@property (strong, nonatomic) NSArray<RecordSessionSegment *> *__nonnull segments;
...
/**
 The recorder that is managing this RecordSession
 */
@property (readonly, nonatomic, weak) RecoderManager *__nullable recorder;
...
/**
 Create a RecordSession
 */
+ (nonnull instancetype)recordSession;

/**
 Create a RecordSession based on dictionary representation
 */
+ (nonnull instancetype)recordSession:(nonnull NSDictionary *)dictionaryRepresentation;

/**
 Calling any method of RecordSession is thread safe. However,
 ...
 access to any modification on this RecordSession.
 */
 ...
 */
- (void)removeSegment:(RecordSessionSegment *__nonnull)segment;
 ...
 */
- (void)addSegment:(RecordSessionSegment *__nonnull)segment;

/**
 Insert a record segment.
 */
- (void)insertSegment:(RecordSessionSegment *__nonnull)segment atIndex:(NSInteger)segmentIndex;
...
/**
 ...
 you should remove the RecordSession from the SCRecorder
 */
...
/**
 Returns a dictionary that represents this RecordSession
...
/**
 ...
 If you don't remove the RecordSession from the SCRecorder while calling this method,
 The SCRecorder might create a new recordSegment right after automatically if it is not paused.
 */
- (BOOL)endSegmentWithInfo:(NSDictionary *__nullable)info completionHandler:(void(^__nullable)(RecordSessionSegment *__nullable segment, NSError *__nullable error))completionHandler;
